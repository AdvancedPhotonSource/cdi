...
PartialCoherence::PartialCoherence(Params *parameters, af::array coherence_array)
{
    ...
    {
        roi_dims = Utils::Int2Dim4(params->GetPcdiRoi());
        d_type c = 0.5;
...
{
    roi_data_abs_2 =  Utils::CenterCropCenter(data, roi_dims);
    if (normalize)
    {
...
{
    roi_amplitudes_prev =  Utils::CenterCropCenter(abs_amplitudes, roi_dims);
}
...
af::array PartialCoherence::ApplyPartialCoherence(af::array abs_amplitudes)
{
    ...
    //printf("converged norm %f\n", sum<d_type>(pow(abs(converged), 2)));
        printf("apply pc took %g seconds\n", timer::stop());
        return converged;
    }
...
void PartialCoherence::UpdatePartialCoherence(af::array abs_amplitudes)
{
    timer::start();
    try{
        af::array roi_abs_amplitudes = Utils::CenterCropCenter(abs_amplitudes, roi_dims);
...
void PartialCoherence::DeconvLucy(af::array amplitudes, af::array data, int iterations)
{
    ...
    try{
        ...
        af::array data_mirror = af::flip(af::flip(af::flip(af::flip(data.copy(), 0),1),2),3);
        af::array convolving = af::array();
        af::array relative_blurr = af::array();
        for (int i = 0; i < iterations; i++)
        {
            convolving = af::fftConvolve(coherence, data);
    ...
    //    printf("coherence norm ,  %f\n", sum<d_type>(pow(abs(coherence), 2)));
        ...
        printf("deconv Lucy pc took %g seconds\n", timer::stop());
    }
    catch(af::exception& e) {
        fprintf(stderr, "%s\n", e.what());
        printf("DeconvLucy\n");
